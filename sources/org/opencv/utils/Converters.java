...
public class Converters {
    ...
    public static void Mat_to_vector_Mat(Mat m, List<Mat> mats) {
        ...
        m.get(0, 0, buff);
        for (int i = 0; i < count; i++) {
            long addr = (buff[i * 2] << 32) | (buff[(i * 2) + 1] & 4294967295L);
    ...
    public static void Mat_to_vector_KeyPoint(Mat m, List<KeyPoint> kps) {
        ...
        if (CvType.CV_64FC(7) == m.type()) {
            ...
            if (m.cols() == 1) {
                ...
                int i2 = 0;
                while (i2 < count) {
                    kps.add(new KeyPoint((float) buff[i2 * 7], (float) buff[(i2 * 7) + i], (float) buff[(i2 * 7) + 2], (float) buff[(i2 * 7) + 3], (float) buff[(i2 * 7) + 4], (int) buff[(i2 * 7) + 5], (int) buff[(i2 * 7) + 6]));
                    i2++;
                    i = 1;
                }
                return;
    ...
    public static void Mat_to_vector_RotatedRect(Mat m, List<RotatedRect> rs) {
        ...
        m.get(0, 0, buff);
        for (int i = 0; i < count; i++) {
            rs.add(new RotatedRect(new Point(buff[i * 5], buff[(i * 5) + 1]), new Size(buff[(i * 5) + 2], buff[(i * 5) + 3]), buff[(i * 5) + 4]));
        }
    }
}
